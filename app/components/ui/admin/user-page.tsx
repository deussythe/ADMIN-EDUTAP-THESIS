...
export function StudentPage() {
    ...
    const handleSubmit = async () => {
        if (
            !formData.name || !formData.lrn || !formData.gradeLevel ||
            !formData.guardianName || !formData.guardianEmail ||
            !formData.contactNumber || !formData.rfidSerial
        ) {
        ...
        if (!editingId) {
            if (!formData.guardianPassword) { alert('Password is required.'); return }
            if (formData.guardianPassword !== formData.confirmPassword) { alert('Passwords do not match.'); return }
            if (formData.guardianPassword.length < 6) { alert('Password must be at least 6 characters.'); return }
        }
        ...
        try {
            ...
            if (editingId && editingStudent) {
                ...
                if (emailChanged && editingStudent.guardianId) {
                    ...
                    alert(
                        'Student updated!\n\n' +
                        '⚠️ Guardian email updated in database. ' +
                        'To update the Firebase Auth login email, deploy the updateGuardianEmail Cloud Function.'
                    )
                } else {
    ...
    if (loading) return (
        <div className="bg-white rounded-lg border border-gray-200 p-8 text-center text-gray-500">
            <Loader className="inline h-5 w-5 animate-spin mr-2" />Loading students...
        </div>
    )
            ...
            <div className="bg-white rounded-lg border border-gray-200 p-8">
                ...
                <div className="flex items-center justify-between mb-6">
                    <div>
                        ...
                        <p className="text-gray-600">Manage student profiles, guardian info, and RFID cards.</p>
                    </div>
                    <button
                        onClick={openAdd}
                                        ...
                                        <td className="py-4 px-4">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-xs text-gray-500">{student.lrn}</p>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="py-4 px-4">
            ...
            {showForm && (
                <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
                    <div className="relative w-full max-w-2xl max-h-[90vh] overflow-y-auto rounded-2xl bg-white p-6 shadow-2xl">
                        ...
                        <div className="flex items-center justify-between mb-6">
                            <div>
                                ...
                                <p className="text-sm text-gray-500">
                                    {editingId ? 'Update student information.' : 'Fill in the details below.'}
                                </p>
                            </div>
                            <button
                        ...
                        {/* Student Details */}
                        <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-3">Student Details</p>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
                            {[
                                { label: 'Full Name', name: 'name', placeholder: 'Juan Dela Cruz' },
                                { label: 'LRN', name: 'lrn', placeholder: '123456789012' },
                                { label: 'School Email', name: 'schoolEmail', placeholder: 'student@school.edu.ph' },
                            ].map(f => (
                                <div key={f.name} className={f.name === 'schoolEmail' ? 'sm:col-span-2' : ''}>
                        ...
                        {/* Guardian Info */}
                        <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-3">Guardian Information</p>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-5">
                            {[
                                { label: 'Guardian Name', name: 'guardianName', placeholder: 'Maria Dela Cruz' },
                                { label: 'Contact Number', name: 'contactNumber', placeholder: '09XX XXX XXXX' },
                            ].map((f: any) => (
                                <div key={f.name}>
                                    <label className="block text-sm font-medium text-gray-700 mb-1">{f.label}</label>
                                    <input
                            ...
                            <div className="sm:col-span-2">
                                ...
                                {editingId && allowEmailEdit && (
                                    <p className="text-xs text-amber-600 mt-1">
                                        ⚠️ Updates the database email. Deploy the Cloud Function to also update the login email.
                                    </p>
                                )}
                            </div>
                        ...
                        {/* RFID */}
                        <p className="text-xs font-semibold uppercase tracking-wider text-gray-400 mb-3">RFID Card</p>
